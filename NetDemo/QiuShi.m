...
- (id)initWithDictionary:(NSDictionary *)dictionary
{
    if (self = [super init])
    {
        ...
        if ((NSNull *)image != [NSNull null]) 
        {
            ...
            NSString *newImageURL = [NSString stringWithFormat:@"http://pic.qiushibaike.com/system/pictures/%@/%@/small/%@",[qiushiID substringToIndex:4],qiushiID,imageURL];
            NSString *newImageMidURL = [NSString stringWithFormat:@"http://pic.qiushibaike.com/system/pictures/%@/%@/medium/%@",[qiushiID substringToIndex:4],qiushiID,imageURL];
